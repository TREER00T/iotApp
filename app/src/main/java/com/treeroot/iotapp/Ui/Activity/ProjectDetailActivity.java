...
public class ProjectDetailActivity extends AppCompatActivity {
    ...
    TextView tv_created, tv_updated, tv_name, tv_project_id;
    View view;
    String id, name, userId, created, updated, isActive;
    Bundle bundle;
            ...
            DialogInterface.OnClickListener dialogClickListener = (dialog, which) -> {
                switch (which) {
                    case DialogInterface.BUTTON_POSITIVE:
                        RequestDeleteProject(Integer.parseInt(id));
                        break;
    ...
    private void RequestDeleteProject(int project_id) {
        ...
        Call<JSONObject> call = req.deleteProject(project_id, shPref.getString(Link.token, null));
        call.enqueue(new Callback<JSONObject>() {
            @Override
            public void onResponse(@NonNull Call<JSONObject> call, @NonNull Response<JSONObject> response) {
                if (response.code() == 200) {
                    pd.dismiss();
                    Toast.makeText(ProjectDetailActivity.this, R.string.success_delete, Toast.LENGTH_SHORT).show();
                    finish();
                }

                if (response.code() == 400 || response.code() == 404) {
                    pd.dismiss();
                    Toast.makeText(ProjectDetailActivity.this, R.string.error_parsing, Toast.LENGTH_SHORT).show();
    ...
    void showProjectDialog() {
        ...
        dialog.setContentView(R.layout.update_project_dialog);
        dialog.setCancelable(true);
        ...
        go.setOnClickListener(v -> {
            ...
            if (str_project.isEmpty()) {
                Toast.makeText(ProjectDetailActivity.this, R.string.error_empty, Toast.LENGTH_SHORT).show();
    ...
    private void RequestUpdateProject(String project_name) {
        ...
        pd.setCancelable(true);
        pd.setCanceledOnTouchOutside(false);
        Call<AddData> call = req.updateProject(Integer.parseInt(id), new AddData(project_name, true), shPref.getString(Link.token, null));
        call.enqueue(new Callback<AddData>() {
            @Override
            public void onResponse(@NonNull Call<AddData> call, @NonNull Response<AddData> response) {
                if (response.code() == 200) {
                    pd.dismiss();
                    Toast.makeText(ProjectDetailActivity.this, R.string.success_updated, Toast.LENGTH_SHORT).show();
                }

                if (response.code() == 400 || response.code() == 404) {
                    pd.dismiss();
                    Toast.makeText(ProjectDetailActivity.this, R.string.invalid_data, Toast.LENGTH_SHORT).show();
    ...
    private void init() {
        ...
        tv_project_id.setText("Project id : " + id);
